                    ...
                    <div class="card" style="width: 20%">
                        ...
                        <div class="card-body">
                            ...
                            <div v-if="agentDetails" class="agentDetails mt-3">
                                <p><strong>DRH:</strong> {{ agentDetails.nom }}  {{ agentDetails.last_name }}</p>
  ...
  computed: {
    ...
    methods: {
        fetchAgentDetails(agentID) {
            ...
            const apiUrl = `http://10.1.44.36:3001/agents/${agentID}`;
        ...
        loadAgents() {
            fetch(`http://10.1.44.36:3001/agents?page=${this.page}&perPage=${this.perPage}`)
                .then(response => response.json())
                .then(data => {
    ...
    checkIfAlreadyManager(agentID) {
            ...
            fetch(`http://10.1.44.36:3001/agents/${agentID}`)
                .then(response => response.json())
                .then(data => {
    ...
    loadAgentCategories() {
        ...
        updateAgent() {
            ...
            fetch(`http://10.1.44.36:3001/agents/${this.editedAgent.agentID}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
...
this.selectedCategories.forEach(categoryID => {
    ...
    fetch(`http://10.1.44.36:3001/gerer?agentID=${agentID}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            entriesToDelete.forEach(entry => {
                fetch(`http://10.1.44.36:3001/gerer/${entry.id}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json'
                    }
